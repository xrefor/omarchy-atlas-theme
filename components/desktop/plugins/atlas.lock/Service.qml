...
Item {
  ...
  property bool screensaverActive: true
  ...
  WlSessionLock {
    ...
    WlSessionLockSurface {
      ...
      LockView {
        ...
        onWakeRequested: {
          root.runWake()
          if (!root.screensaverActive) root.armScreensaverResume()
        }
        onScreensaverDismissed: root.dismissScreensaver()
      }
  ...
  Timer {
    id: screensaverResumeTimer
    interval: 20000
    repeat: false
    onTriggered: root.resumeScreensaver()
  }
  ...
  onAuthenticatingPasswordChanged: {
    if (!lockRequested) return
    if (authenticatingPassword) screensaverResumeTimer.stop()
    else root.runWake()
  }
