...
Item {
  ...
  // Injected by omarchy-shell; cloned plugins receive the public shell API.
  ...
  function startIdleCycle() {
    ...
    // Session-lock first. The lock surface owns the rain HUD; foot/ttfx cannot
    // paint through ext-session-lock, and embedding WebEngine in the lock
    // process aborts omarchy-shell.
    root.lockSystem("idle")
  }
  ...
  function handleScreensaverWindowClosed(address) {
    ...
    // Key or mouse ended the HUD. Lock immediately so the password tile
    // covers the desktop; omarchy-system-lock tears down remaining HUD windows.
