...
Panel {
  ...
  function sectionCount(section) {
    ...
    if (section === "nightlight") return 0
  ...
  function sectionIsSingleRow(section) {
    // brightness, night light, and text size are lone rows; scale presets sit horizontally.
    return section === "brightness" || section === "nightlight" || section === "textsize" || section === "scale"
  }

  function sectionFirstIndex(section) {
    if (section === "brightness" || section === "nightlight" || section === "textsize") return -1
    return 0
  }
  ...
  function clampCursor() {
    ...
    if (sectionIsSingleRow(focusSection)) {
      ...
      if (focusSection === "brightness" || focusSection === "nightlight" || focusSection === "textsize") selectedIndex = -1
      ...
      ScrollView {
        ...
        Column {
          ...
          Toggle {
            ...
            accent: Color.accent
            fontFamily: root.bar.fontFamily
            hasCursor: root.cursorActive && root.focusSection === "nightlight"
            onHasCursorChanged: if (hasCursor) root.ensureCursorVisible(nightlightRow)
            onClicked: root.toggleNightlight()
            onHovered: function(on) {
              if (!on || root.reflowingText) return
              root.cursorActive = true
              root.focusSection = "nightlight"
              root.selectedIndex = -1
            }
          }
