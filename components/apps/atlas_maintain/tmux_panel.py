"""Detached read-only maintenance panel owned by one originating tmux pane."""
...
def _private_directory(path):
    ...
    if (not stat.S_ISDIR(info.st_mode) or info.st_uid != os.getuid()
        ...
        raise OSError(f'ATLAS Maintain runtime directory must be owned and private: {path}')
    return path
    ...
    if runtime and Path(runtime).is_absolute():
        try:
            ...
            if (stat.S_ISDIR(info.st_mode) and info.st_uid == os.getuid()
                ...
                return _private_directory(Path(runtime) / 'atlas-maintain')
    return _private_directory(Path('/tmp') / f'atlas-maintain-{os.getuid()}')
...
class Panel:
    ...
    def _lock(self):
        ...
        try:
            ...
            if (not stat.S_ISDIR(info.st_mode) or info.st_uid != os.getuid()
                    or stat.S_IMODE(info.st_mode) != 0o700):
                raise OSError('ATLAS Maintain runtime directory is no longer private')
            descriptor = os.open(self.lock_name, os.O_RDWR | os.O_CREAT | os.O_NOFOLLOW,
                                 0o600, dir_fd=directory)
            info = os.fstat(descriptor)
            if (not stat.S_ISREG(info.st_mode) or info.st_uid != os.getuid()
                    or stat.S_IMODE(info.st_mode) != 0o600 or info.st_nlink != 1):
                raise OSError('ATLAS Maintain panel lock must be owned and private')
    ...
    def open(self):
        with self._lock():
            ...
            if columns is None:
                pane = self._tmux('new-window', '-d', '-t', session + ':', '-n', 'maintain',
                                  '-P', '-F', '#{pane_id}', '--', *self.command)
            else:
                pane = self._tmux('split-window', '-d', '-h', '-l', str(columns), '-t', self.origin,
                                  '-P', '-F', '#{pane_id}', '--', *self.command)
            try:
                self._tmux('set-option', '-p', '-t', pane, '@atlas_maintain_origin', self.origin)
