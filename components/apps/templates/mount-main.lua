...
function M:redraw()
	...
	if #rows == 0 then rows[1] = ui.Row { "—", "No USB drives", "Connect a drive to browse", "—" } end
	return {
		ui.Clear(self._area),
		ui.Border(ui.Edge.ALL)
			:area(self._area)
			:type(ui.Border.PLAIN)
			:style(ui.Style():fg("{{ accent }}"))
			:title(ui.Line(" DRIVES / m mount · u unmount · e eject "):align(ui.Align.CENTER)),
		ui.Table(rows)
			:area(self._area:pad(ui.Pad(1, 2, 1, 2)))
			:header(ui.Row({ "DEVICE", "LABEL", "MOUNTPOINT", "FORMAT" }):style(ui.Style():bold()))
			:row(self.cursor)
			:row_style(ui.Style():fg("{{ accent }}"):underline())
			:widths {
				ui.Constraint.Percentage(22),
				ui.Constraint.Percentage(23),
...
function M.obtain()
	local tbl = {}
	local last
	local removable = require(".cross").removable_sources()
	for _, p in ipairs(fs.partitions()) do
		if removable[p.src] then
