...
def _private_directory(path):
    ...
    if (not stat.S_ISDIR(info.st_mode) or info.st_uid != os.getuid()
        ...
        raise OSError(f'ATLAS Frontier runtime directory must be owned and private: {path}')
    return path
    ...
    if runtime and Path(runtime).is_absolute():
        try:
            ...
            if (stat.S_ISDIR(info.st_mode) and info.st_uid == os.getuid()
                ...
                return _private_directory(Path(runtime) / 'atlas-frontier')
    return _private_directory(Path('/tmp') / f'atlas-frontier-{os.getuid()}')


class Panel:
    """Open or close only the Frontier pane belonging to an exact origin."""
    ...
    def _lock(self):
        ...
        try:
            ...
            if (not stat.S_ISREG(info.st_mode) or info.st_uid != os.getuid()
                ...
                raise OSError('ATLAS Frontier panel lock must be owned and private')
    ...
    def open(self):
        with self._lock():
            ...
            if panel_columns is None:
                pane = self._tmux('new-window', '-d', '-t', session + ':', '-n', 'frontier',
                                  '-P', '-F', '#{pane_id}', '--', *self.command)
            else:
                pane = self._tmux('split-window', '-d', '-h', '-l', str(panel_columns),
                                  '-t', self.origin, '-P', '-F', '#{pane_id}', '--', *self.command)
            try:
                self._tmux('set-option', '-p', '-t', pane,
                           '@atlas_frontier_origin', self.origin)
