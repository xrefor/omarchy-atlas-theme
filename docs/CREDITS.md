...
earlier theme with the image generation tool. The original generation prompts
...
Ember Causeway and Obsidian Fold were generated with the built-in image generation
tool on September 15, 2026; their prompts and native dimensions are recorded in
[wallpaper additions](WALLPAPER-ADDITIONS-2026-09-15.md). No game media, personal
photographs or unrelated stock wallpapers are included.

`docs/SOURCE-INVENTORY.json` records the selected original source assets and
hashes. It is an inventory of source files, not a dump of home configuration.
Generated/modified distribution files are covered by the release SHA256SUMS.
